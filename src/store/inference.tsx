...
export const InferenceContextProvider = ({
  ...
  const updateSelectedModel = useCallback(
    (config: Configuration) => {
      ...
      // If user has a saved model, use it
      if (config.model) {
        const selectedModel = state.models.find((m) => m.id === config.model) || null;
        dispatch({
          type: InferenceActionTypes.SET_SELECTED_MODEL,
      ...
      // No model saved — auto-select the first model in the list
      dispatch({
        type: InferenceActionTypes.SET_SELECTED_MODEL,
        payload: state.models[0],
      });
    },
    [state.models]
  );
  ...
  // Auto-persist: when a model is auto-selected (config.model is empty but selectedModel exists),
  // save it to config so it persists across reloads
  useEffect(() => {
    if (state.selectedModel && !config.model) {
      saveConfig({ ...config, model: state.selectedModel.id });
    }
    // Also handle case where saved model no longer exists — auto-select first
    if (config.model && state.models.length > 0 && !state.models.find(m => m.id === config.model) && state.selectedModel) {
      saveConfig({ ...config, model: state.selectedModel.id });
    }
  }, [state.selectedModel, state.models, config, saveConfig]);
