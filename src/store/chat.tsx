...
enum ChatActionType {
  ...
  SET_LIVE_TOKEN_STATS = 'SET_LIVE_TOKEN_STATS',
  REMOVE_LIVE_TOKEN_STATS = 'REMOVE_LIVE_TOKEN_STATS',
}
...
interface SetLiveTokenStatsAction {
  type: ChatActionType.SET_LIVE_TOKEN_STATS;
  payload: { convId: string; stats: LiveTokenStats };
}

interface RemoveLiveTokenStatsAction {
  type: ChatActionType.REMOVE_LIVE_TOKEN_STATS;
  payload: { convId: string };
}
...
type ChatAction =
  ...
  | SetCanvasDataAction
  | SetLiveTokenStatsAction
  | RemoveLiveTokenStatsAction;
...
interface ChatState {
  ...
  liveTokenStats: Record<Conversation['id'], LiveTokenStats>;
}
...
const chatReducer = (state: ChatState, action: ChatAction): ChatState => {
  switch (action.type) {
    ...
    case ChatActionType.SET_LIVE_TOKEN_STATS:
      return {
        ...state,
        liveTokenStats: {
          ...state.liveTokenStats,
          [action.payload.convId]: action.payload.stats,
        },
      };
    case ChatActionType.REMOVE_LIVE_TOKEN_STATS: {
      const newStats = { ...state.liveTokenStats };
      delete newStats[action.payload.convId];
      return {
        ...state,
        liveTokenStats: newStats,
      };
    }
    default:
      return state;
...
interface ChatContextValue {
  ...
  continueMessage: (msg: Message, onChunk: (id?: number) => void) => Promise<void>;
  regenerateWithModel: (msg: Message, model: string, onChunk: (id?: number) => void) => Promise<void>;
...
export const ChatContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setLiveTokenStats = useCallback(
    (convId: string, stats: LiveTokenStats | null) => {
      if (!stats) {
        dispatch({
          type: ChatActionType.REMOVE_LIVE_TOKEN_STATS,
          payload: { convId },
  ...
  return (
    <ChatContext.Provider
      value={{
        ...
        pendingMessages: state.pendingMessages,
        liveTokenStats: state.liveTokenStats,
        isGenerating,
        sendMessage,
        stopGenerating,
        replaceMessage,
