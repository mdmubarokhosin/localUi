...
export interface Message {
  ...
  parent: Message['id'];
  children: Message['id'][];
  // tool_use support for agentic AI loop
  tool_calls?: ToolCall[];
  // streaming metadata for live stats
  streamStartTime?: number;
}

/** Tool call from assistant message (agentic AI loop) */
...
export interface MessageExtraVideoFile {
  type: 'videoFile';
  name: string;
  base64Data: string;
  mimeType: string;
...
export interface Conversation {
  ...
  name: string;
  pinned?: boolean; // conversation pinning
  branchParentId?: string; // parent conversation ID for tree view
}
